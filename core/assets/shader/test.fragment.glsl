...
vec2 posOffset;
float offsetShadow = 0.0;

uniform int u_renderFlag;
uniform float u_globalTime;
uniform float u_blockTextureSize;
...
#ifdef tangentFlag
#ifdef normalTextureFlag
	vec3 n = texture2D(u_normalTexture, v_normalUV+posOffset).rgb;
...
vec3 specularReflection(PBRSurfaceInfo pbrSurface, PBRLightInfo pbrLight) {
	return pbrSurface.reflectance0
			+ (pbrSurface.reflectance90 - pbrSurface.reflectance0)
					* pow(clamp(1.0 - pbrLight.VdotH, 0.0, 1.0), 5.0);
}
...
float microfacetDistribution(PBRSurfaceInfo pbrSurface, PBRLightInfo pbrLight) {
	...
	float f = (pbrLight.NdotH * roughnessSq - pbrLight.NdotH) * pbrLight.NdotH
			+ 1.0;
...
#ifdef diffuseTextureFlag
	vec4 baseColor = SRGBtoLINEAR(texture2D(u_diffuseTexture, v_diffuseUV+posOffset)) * baseColorFactor;
...
vec3 getLightContribution(PBRSurfaceInfo pbrSurface, vec3 l) {
	...
	PBRLightInfo pbrLight = PBRLightInfo(NdotL, NdotH, LdotH, VdotH);
...
float getGrey(vec4 inputColor) {
	return (inputColor.r * .33 + inputColor.g * .33 + inputColor.b * .33)-0.5;
}



void main() {
if(v_position.y < u_seaLevel){
	discard;
}
	...
	//def block flag
	if (u_renderFlag==0) {
		vec3 viewPos = normalize(v_TBN*(u_cameraPosition.xyz-v_position));
		...
		if (viewPos.z>0.0) {
			vec2 tSize=vec2(textureSize(u_normalTexture,0));
			vec2 textureSizeOne=1.0/tSize;

			vec2 offsetBoundMax;
			vec2 offsetBoundMin;

			//Block Texture Mix Map

			if (tSize.x==tSize.y) {
				//OneBlockBound
#ifdef diffuseTextureFlag
				if(length(viewPos)<30) {
					offsetBoundMin=floor(v_diffuseUV*tSize*u_blockTextureSize)*u_blockTextureSize-v_diffuseUV+vec2(textureSizeOne)/2.0;
					offsetBoundMax=offsetBoundMin+u_blockTextureSize-vec2(textureSizeOne);

					float layerMin=10.0;
					float layerMax=100.0;
					float maxHeight=2.0;
					float layerNum=clamp(round(20/viewPos.z) - length(viewPos)*10,layerMin,layerMax);
					float layer=layerNum;
					vec3 useLong=(maxHeight/viewPos.z)*viewPos/layerNum;

					vec3 lastPoint=layer*useLong;
					lastPoint.z-=maxHeight/2.0;

					float pointTrulyHight;
					float lastHeight=0.0;
					while(layer>=0.0) {
						vec3 nowPoint=layer*useLong;
						nowPoint.z-=maxHeight/2.0;
						//float height=texture2D(u_normalTexture, v_normalUV+nowPoint.xy*textureSizeOne).a/(1/maxHeight);
						float height=(getGrey(texture2D(u_diffuseTexture, v_diffuseUV+nowPoint.xy*textureSizeOne)))*maxHeight;
						if(nowPoint.z<height) {
							//Satisfy zero formula
							if (layer==layerNum) {
								posOffset=clamp(nowPoint.xy*textureSizeOne,offsetBoundMin,offsetBoundMax);
								break;
							}
							int maxTimes=5;
							vec3 minValuePoint=nowPoint;
							float minValue=height;
							vec3 maxValuePoint=lastPoint;
							float maxValue=lastHeight;

							vec3 dis=maxValuePoint-minValuePoint;

							float factor=2.0;
							while(maxTimes>0) {
								vec3 middleValuePoint=maxValuePoint-dis/factor;
								float middleValue=(getGrey(texture2D(u_diffuseTexture, v_diffuseUV+middleValuePoint.xy*textureSizeOne)))*maxHeight;
								if((maxValuePoint.z-maxValue)*(middleValuePoint.z-middleValue)<0.0) {
									minValuePoint=middleValuePoint;
									minValue=middleValue;
								} else {
									maxValuePoint=middleValuePoint;
							...
							pointTrulyHight=minValuePoint.z;
							if(repeat==0) {
								posOffset=mod(minValuePoint.xy*textureSizeOne-offsetBoundMin,offsetBoundMax-offsetBoundMin)+offsetBoundMin;
							} else {
								posOffset=clamp(minValuePoint.xy*textureSizeOne,offsetBoundMin,offsetBoundMax);
							}
							break;
						}
						lastPoint=nowPoint;
					...
					vec3 lightDic=v_TBN*(-u_dirLights[0].direction);
					if (repairTBN==1) {
						lightDic.z =-lightDic.z;
						lightDic.x =-lightDic.x;
						lightDic.y =lightDic.y;
					}
					if(lightDic.z>=0) {
						int ceilNum=30;
						int ceil=ceilNum;
						lightDic=maxHeight/lightDic.z*lightDic/ceil;
						//float hei= getGrey(texture2D(u_diffuseTexture,v_diffuseUV+posOffset))*maxHeight;
						while(ceil>0.0) {
							vec3 dic=ceil*lightDic;
							float nowHeight;
							if(repeat==0) {
								nowHeight=(getGrey( texture2D(u_diffuseTexture,mod(dic.xy*textureSizeOne+posOffset,offsetBoundMax-offsetBoundMin)+offsetBoundMin+ v_diffuseUV)))*maxHeight;
							} else {
								nowHeight=(getGrey( texture2D(u_diffuseTexture,clamp(dic.xy*textureSizeOne+posOffset,offsetBoundMin,offsetBoundMax)+ v_diffuseUV)))*maxHeight;

							}

							if(nowHeight>=dic.z+pointTrulyHight) {
								offsetShadow=1.0;
								break;
							}
							ceil--;
				...
				//ani uv
				float blockW=tSize.x;
				posOffset=vec2(0.0,(mod(u_globalTime*50.0,tSize.y/blockW))*(1.0/blockW));
			}
		}
	}
	...
	vec4 mrSample = texture2D(u_MetallicRoughnessSampler, v_metallicRoughnessUV+posOffset);
	...
	/*
	 #ifdef normalTextureFlag
	 baseColor=vec4((vec3(0.0,0.0,1.0)*v_TBN+1.0)/2.0,1.0);
	 #endif
	 */
	vec3 f0 = vec3(0.04);
	...
	PBRSurfaceInfo pbrSurface = PBRSurfaceInfo(n, v, NdotV, perceptualRoughness,
	...
	color = diffuseColor * ambientColor + color * getShadow() * NdotL0;
	//color = diffuseColor * ambientColor*6;
#else
...
#ifdef occlusionTextureFlag
	float ao = texture2D(u_OcclusionSampler, v_occlusionUV+posOffset).r;
	color = mix(color, color * ao, u_OcclusionStrength);
#endif

	// Add emissive
#if defined(emissiveTextureFlag) && defined(emissiveColorFlag)
	vec3 emissive = SRGBtoLINEAR(texture2D(u_emissiveTexture, v_emissiveUV+posOffset)).rgb * u_emissiveColor.rgb;
#elif defined(emissiveTextureFlag)
	vec3 emissive = SRGBtoLINEAR(texture2D(u_emissiveTexture, v_emissiveUV+posOffset)).rgb;
